...
<template>
  ...
  >
    <template #content>
      <div
        class="p-3 border-b border-surface-700 flex items-center justify-between"
      >
        <h3 class="text-sm font-semibold">Chat History</h3>
        <i
          v-tooltip="'Export All'"
      ...
      <div class="flex-1 overflow-auto">
        ...
        <div
          ...
          :class="{ 'bg-surface-700': currentChatId === chat.id }"
        ...
        >
          <div v-if="editingChatId === chat.id" class="flex items-center gap-2">
            <InputText
              ...
              @update:model-value="emit('updateEditTitle', $event)"
              @keyup.enter="emit('saveEdit')"
              @keyup.escape="emit('cancelEdit')"
              @click.stop
            />
            <i
              class="fas fa-check text-green-400 hover:text-green-300 cursor-pointer"
              @click.stop="emit('saveEdit')"
            />
            <i
              class="fas fa-times text-surface-400 hover:text-white cursor-pointer"
        ...
        <div
          v-if="chatHistory.length === 0 && !isProjectChanging"
          class="p-4 text-center text-surface-400 text-sm"
        >
          No chat history yet
        </div>
